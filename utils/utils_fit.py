...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, loss_history_seg, eval_callback, eval_callback_seg, optimizer, epoch, epoch_step,
    ...
    if local_rank == 0:
        ...
        if (epoch + 1) % save_period == 0 or epoch + 1 == Epoch:
            ...
            epoch + 1, loss + loss_seg / epoch_step, val_loss / epoch_step_val, val_loss_seg / epoch_step_val)))

        if len(loss_history.val_loss) <= 1 or (val_loss + val_loss_seg / epoch_step_val) <= min(loss_history.val_loss) + min(loss_history_seg.val_loss):
